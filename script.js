...
function renderPassportPhoto(ctx, targetWidth, targetHeight) {
...
function generateSheet() {
  ...
  for (let row = 0; row < 3; row += 1) {
    for (let col = 0; col < 2; col += 1) {
      finalCtx.save();
      finalCtx.beginPath();
      finalCtx.rect(col * PHOTO_SIZE, row * PHOTO_SIZE, PHOTO_SIZE, PHOTO_SIZE);
      finalCtx.clip();
      renderPassportPhoto(
        finalCtx,
        PHOTO_SIZE,
      ...
      finalCtx.strokeRect(col * PHOTO_SIZE + 5, row * PHOTO_SIZE + 5, PHOTO_SIZE - 10, PHOTO_SIZE - 10);
    }
  }
